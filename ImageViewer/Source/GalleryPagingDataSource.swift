...
final class GalleryPagingDataSource: NSObject, UIPageViewControllerDataSource {
    ...
    func createItemController(_ itemIndex: Int, isInitial: Bool = false) -> UIViewController {
        ...
        case .video(let fetchImageBlock, let fetchVideoURLBlock):

            let videoController = VideoViewController(index: itemIndex, itemCount: itemsDataSource.itemCount(), fetchImageBlock: fetchImageBlock, fetchVideoURLBlock: fetchVideoURLBlock, scrubber: scrubber, configuration: configuration, isInitialController: isInitial)
